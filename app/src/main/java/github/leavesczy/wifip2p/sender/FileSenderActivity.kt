...
class FileSenderActivity : BaseActivity() { // BaseActivity를 상속받음
    ...
    private fun sendUnicastMessage(message: String, targetIpAddress: String) {
        ...
        val sendtimestamp = SimpleDateFormat("HH:mm:ss.SSS", Locale.getDefault()).format(Date())

        if (targetIpAddress.isNotBlank()) {
            fileSenderViewModel.sendMessage(ipAddress = targetIpAddress, message = "0UDPuni "+sendtimestamp+message)

            // 송신된 메시지를 화면에 추가 (14글자까지만 표시)
            val truncatedMessage = if (message.length > 14) message.take(14) else message
            sentMessageCount += 1
            val formattedMessage = "[$sentMessageCount] $truncatedMessage \n at $sendtimestamp"
    ...
    private fun sendTcpMessage(message: String, targetIpAddress: String) {
        ...
        if (targetIpAddress.isNotBlank()) {
            fileSenderViewModel.sendMessage_TCP(ipAddress = targetIpAddress, message = "0TCP"+message)
    ...
    private fun sendMessage(message: String) {
        ...
        if (!ipAddress.isNullOrBlank()) {
            ...
            fileSenderViewModel.sendMessage(ipAddress = ipAddress, message = "0udpbroad "+timestamp + message)
            // 송신된 메시지를 화면에 추가 (20글자까지만 표시)
            val truncatedMessage = if (message.length > 12) message.take(12) else message
            sentMessageCount += 1
    ...
    private fun initView() { // UI요소들 설정
        ...
        btnSendTcpMessage.setOnClickListener{
            ...
            if (etTargetTimeInterval.text.toString().isBlank() || etTargetTimeRange.text.toString().isBlank())
                sendTcpMessage("0" + largeMessage, targetIpAddress) //timestamp 지움, 0이 여기에 필요한가?
            else
        ...
        btnSendUnicastProgMessage.setOnClickListener{
            ...
            if (targetIpAddress.isNotBlank()) {
                ...
                if (targettimeinterval.isBlank() || targettimerange.isBlank())
                    sendUnicastMessage("$messageNumber: " + timestamp + message, targetIpAddress)
                else
        ...
        lifecycleScope.launch {
            ...
            launch {
                fileReceiverViewModel.receivedMessage.collect { (message, senderAddress) ->
                    ...
                    val timestamp = SimpleDateFormat("HH:mm:ss.SSS", Locale.getDefault()).format(Date())
                    val formattedMessage = "[$receiveMessageCount] $message \n from $senderAddress  \n ($timestamp)"
